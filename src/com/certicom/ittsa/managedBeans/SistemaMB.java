...
public class SistemaMB  extends GenericBeans implements Serializable{
   ...
   // private Menu menu;
    ...
    public void nuevoSistema(){
    	this.sistema = new Sistema();
    	this.editar = Boolean.FALSE;
    	this.titulo = "Registrar nuevo módulo";
    }
    
    public void editarSistema(Sistema s){
    	this.editar = Boolean.TRUE;
    	this.sistema = s;
    	this.titulo = "Modificar módulo";
    }
    ...
    public void guardarSistema() throws Exception {
    	 ...
    	 if(this.editar){ //editando
    		 ...
    		 this.editarSistema();
    		 FacesUtils.showFacesMessage("Módulo actualizado correctamente.",Constante.INFORMACION);
    		 ...
    		 if(i==0){
	             ...
	             log.setDescripcion("Se insertó el sistema: " + sistema.getNombre_sistema());
	             logmb.insertarLog(log);
	             this.sistema =new Sistema();
	             FacesUtils.showFacesMessage("Módulo registrado correctamente.",Constante.INFORMACION);
	        	 context.update("msgGeneral");
	        	 context.execute("dlgNuevo.hide()");
			 }else{
				 //context.addCallbackParam("esAviso", Boolean.TRUE );
				 System.out.println("entro en este metodo");
				 FacesUtils.showFacesMessage("Módulo se encuentra ya registrado.",Constante.ERROR);
   ...
   public void eliminarSistema(){
        ...
        try {
        	...
        	if(listaMenus.size()>0){
    			...
    			listaSistema = sistemaServices.listSistema();
    	        FacesUtils.showFacesMessage("Módulo eliminado correctamente.",Constante.INFORMACION);
    	        
    	        log.setAccion("DELETE");
                log.setDescripcion("Se eliminó el sistema: " + sistema.getNombre_sistema());
                logmb.insertarLog(log);
        	}
 
		} catch (Exception e) {
			FacesUtils.showFacesMessage("Módulo no se puede eliminar, posiblemente tenga menúes asociados.",Constante.ERROR);
			e.printStackTrace();
		}
...
*/
    ...
    public void agregarMenu(){
		try{
			...
			setMenu(new Menu());
			setId_menu(0);
			FacesUtils.showFacesMessage("Menú registrado correctamente.", 3);
