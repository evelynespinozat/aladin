...
@ManagedBean(name="mantAutoparteMB")
@ViewScoped
public class MantenimientoAutoparteMB extends GenericBeans implements Serializable{
	...
	public MantenimientoAutoparteMB(){;}
	...
	public void incia()
	{
		...
		this.operation = (String) flash.get("operation");
	...
	public void rellenadoListas(){
		try {
			this.listaHistorialAuto = this.historialMantenimientoService.obtenerAutopartesConMecanico(flota);
			List<FlotaAutoparte> lista = new ArrayList<>();
			lista = this.flotaAutoparteService.autopartesxBus(flota);
	...
	public void asignarMecanico(FlotaAutoparte flotaAutoparte){
		try {
			...
			if(this.operation.equals("O")){
				this.historialMantenimiento.setKmRecibido(flotaAutoparte.getKmActual());
			}else{
				this.historialMantenimiento.setKmRecibido(flotaAutoparte.getKmAproximado());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void guardarHistorial(){
			...
			if(this.desAutoparte != ""){
				try {	
					this.historialMantenimiento.setTipoServicio("PREVENTIVO");
	...
	public void guardarObservaciones(){
		try {
			...
			if(this.historialMantenimiento2.getTipMantEfectuado().equals("REMPLAZADO")){
				...
				//cambiamos el km de la pieza a cero y registrar el UltimoKmCambio
	...
	public Boolean getVisibleCantidad() {
